...
class Repro(torch.nn.Module):
    ...
    def forward(self, arg0_1):
        view = torch.ops.aten.view.default(arg0_1, [500, 128, 1, 4]);  arg0_1 = None
        full = torch.ops.aten.full.default([500, 128, 1, 16], 0, dtype = torch.float32, layout = torch.strided, device = device(type='cuda', index=0), pin_memory = False)
        slice_1 = torch.ops.aten.slice.Tensor(full, 3, 1, 5)
        copy = torch.ops.aten.copy.default(slice_1, view);  slice_1 = view = None
        slice_scatter = torch.ops.aten.slice_scatter.default(full, copy, 3, 1, 5);  full = copy = None
        view_2 = torch.ops.aten.view.default(slice_scatter, [500, 128, 16]);  slice_scatter = None
        return (view_2,)
