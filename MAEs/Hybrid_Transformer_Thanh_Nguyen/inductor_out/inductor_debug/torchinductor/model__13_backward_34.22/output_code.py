...
# kernel path: /traces/inductor_cache/ju/cjugndyqzdvrbooj6rl3pfi5cqlkwnee74kvnibgpor3mk7hnir5.py
# Topologically Sorted Source Nodes: [sum_1], Original ATen: [aten.sum]
# Source node to ATen node mapping:
#   sum_1 => sum_1
# Graph fragment:
#   %tangents_1 : Tensor "f32[500, 128, 1, 16][2048, 16, 16, 1]cuda:0" = PlaceHolder[target=tangents_1]
#   %sum_1 : Tensor "f32[1, 1, 1, 16][16, 16, 16, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.sum.dim_IntList](args = (%tangents_1, [0, 1], True), kwargs = {})
#   return %buf0
...
def triton_red_fused_sum_0(in_ptr0, out_ptr0, xnumel, r0_numel, XBLOCK : tl.constexpr, R0_BLOCK : tl.constexpr):
    ...
    for r0_offset in range(0, r0_numel, R0_BLOCK):
        ...
        rindex = r0_index
        r0_2 = r0_index
        tmp0 = tl.load(in_ptr0 + (x0 + 16*r0_2 + 2048*x1), r0_mask & xmask, eviction_policy='evict_first', other=0.0)
...
# kernel path: /traces/inductor_cache/u5/cu5b4uj3hjtgfs5xp6cebqj3kek3amhzsddfkv33ukjxh25arbfp.py
# Topologically Sorted Source Nodes: [sum_1], Original ATen: [aten.sum]
# Source node to ATen node mapping:
#   sum_1 => sum_1
# Graph fragment:
#   %buf0 : Tensor "f32[1, 1, 1, 16, 500][8000, 8000, 8000, 1, 16]cuda:0" = PlaceHolder[target=buf0]
#   %sum_1 : Tensor "f32[1, 1, 1, 16][16, 16, 16, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.sum.dim_IntList](args = (%tangents_1, [0, 1], True), kwargs = {})
#   return %sum_1
...
@triton_heuristics.reduction(
    ...
    inductor_meta={'grid_type': 'Grid1D', 'autotune_hints': set(), 'kernel_name': 'triton_red_fused_sum_1', 'mutated_arg_names': [], 'optimize_mem': True, 'no_x_dim': False, 'atomic_add_found': False, 'num_load': 1, 'num_store': 1, 'num_reduction': 1, 'backend_hash': 'D221555120843152094FD7E43096F924AB4102579828DDF622EEF3389D4AEF7F', 'assert_indirect_indexing': True, 'autotune_local_cache': True, 'autotune_pointwise': True, 'autotune_remote_cache': None, 'force_disable_caches': False, 'dynamic_scale_rblock': True, 'max_autotune': False, 'max_autotune_pointwise': False, 'min_split_scan_rblock': 256, 'spill_threshold': 16, 'store_cubin': False, 'deterministic': False, 'force_filter_reduction_configs': False, 'are_deterministic_algorithms_enabled': False, 'tiling_scores': {'x': 32128, 'r0_': 0}}
)
@triton.jit
...
class Runner:
    ...
    def call(self, args):
        tangents_1, = args
        args.clear()
        assert_size_stride(tangents_1, (500, 128, 1, 16), (2048, 16, 16, 1))
        with torch.cuda._DeviceGuard(0):
            torch.cuda.set_device(0)
            buf0 = empty_strided_cuda((1, 1, 1, 16, 500), (8000, 8000, 8000, 1, 16), torch.float32)
            # Topologically Sorted Source Nodes: [sum_1], Original ATen: [aten.sum]
            # [Provenance debug handles] triton_red_fused_sum_0:280
            stream0 = get_raw_stream(0)
            triton_red_fused_sum_0.run(tangents_1, buf0, 8000, 128, stream=stream0)
            buf1 = empty_strided_cuda((1, 1, 1, 16), (16, 16, 16, 1), torch.float32)
            # Topologically Sorted Source Nodes: [sum_1], Original ATen: [aten.sum]
            # [Provenance debug handles] triton_red_fused_sum_1:281
            stream0 = get_raw_stream(0)
            triton_red_fused_sum_1.run(buf0, buf1, 16, 500, stream=stream0)
            del buf0
        return (reinterpret_tensor(buf1, (1, 1), (16, 1), 0), tangents_1, )
...
def benchmark_compiled_module(times=10, repeat=10):
    ...
    tangents_1 = rand_strided((500, 128, 1, 16), (2048, 16, 16, 1), device='cuda:0', dtype=torch.float32)
    fn = lambda: call([tangents_1])
