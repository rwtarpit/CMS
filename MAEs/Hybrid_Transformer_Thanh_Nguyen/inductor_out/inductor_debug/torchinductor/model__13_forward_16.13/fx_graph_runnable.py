...
class Repro(torch.nn.Module):
    ...
    def forward(self, primals_1, primals_2):
        full_default = torch.ops.aten.full.default([1, 15], 0, dtype = torch.float32, layout = torch.strided, device = device(type='cuda', index=0), pin_memory = False)
        cat = torch.ops.aten.cat.default([primals_1, full_default], -1);  primals_1 = full_default = None
        add = torch.ops.aten.add.Tensor(primals_2, cat);  primals_2 = cat = None
        return (add,)
        
def load_args(reader):
    buf0 = reader.storage(None, 4, device=device(type='cuda', index=0))
    reader.tensor(buf0, (1, 1), is_leaf=True)  # primals_1
    buf1 = reader.storage(None, 4096000, device=device(type='cuda', index=0))
    reader.tensor(buf1, (500, 128, 1, 16), is_leaf=True)  # primals_2
